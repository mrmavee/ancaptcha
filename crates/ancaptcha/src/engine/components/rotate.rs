...
pub fn generate_rotate_html(config: &mut RotateConfig) -> String {
    ...
    for step in 0..steps {
        let _stage_id = config.mapper.get_or_create(&format!("st{step}"));
        let _ = write!(html, r#"<div class="{stage_class} s{step}">"#);

        let _ = write!(html, r#"<div class="ti">ROTATE THE PIC</div>"#);
        ...
        for idx in 0..8 {
            ...
            let _ = write!(
                html,
                r#"<label for="{next_id}" class="{label_class} {btn_class} rt">Rotate</label>"#
            );
        }
...
pub fn generate_rotate_css(config: &mut RotateConfig) -> String {
    ...
    let _ = write!(
        css,
        ".im-w{{width:120px;height:120px;margin:0 auto 10px;border-radius:50%;overflow:hidden;border:3px solid {accent_color};position:relative;}}"
    );
    let _ = write!(
        css,
    ...
    let _ = write!(
        css,
        ".{wrapper_class}{{width:100%;height:100%;transition:transform 0.3s;}}"
    );
    ...
    for (step, img_b64) in config.images_base64.iter().enumerate() {
        ...
        let _ = write!(
            css,
            ".anc_{} .{img_data_class}{{width:100%;height:100%;background-image:url(data:image/jpeg;base64,{img_b64});background-size:cover;transition:transform 0.3s;pointer-events:none;user-select:none;-webkit-user-drag:none;}}",
            config.token
        );
    }
    ...
    let _ = write!(css, ".{controls_class} .rt{{display:none;}}");
...
fn write_rotate_controls_css(
    ...
    let _ = write!(
        css,
        ".{controls_class}{{display:flex;flex-direction:column;gap:10px;width:100%;}}"
    );
    let _ = write!(
        css,
        ".{btn_class}{{display:block;width:100%;padding:10px;border:none;border-radius:4px;background:{bg_color};color:{text_color};border:1px solid {border_color};font-weight:bold;cursor:pointer;text-align:center;box-sizing:border-box;text-decoration:none;transition:background 0.2s,transform 0.1s;user-select:none;}}"
    );
    let _ = write!(css, ".{btn_class}:hover{{opacity:0.8;}}");
    let _ = write!(css, ".{btn_class}:active{{transform:scale(0.98);}}");
    let _ = write!(
        css,
        ".nx{{background:{accent_color};color:{bg_color};border:none;}}"
    );
    let _ = write!(css, ".nx:hover{{opacity:0.9;}}");
    let _ = write!(css, ".nx:active{{transform:scale(0.98);}}");
}
...
fn write_rotate_step_css(css: &mut String, config: &mut RotateConfig, step: u8, stage_class: &str) {
    ...
    let _ = write!(
        css,
        ".{initial_rot_class}{{transform:rotate({initial_rot}deg);}}"
    );
    ...
    for idx in 0..8 {
        ...
        let _ = write!(
            css,
            "#{input_id}:checked~.{stage_class}.s{step} .im-w div#{img_id_obf}{{transform:rotate({angle}deg);}}"
        );
        let _ = write!(
            css,
            "#{input_id}:checked~.{stage_class}.s{step} .rt.{label_class} {{display:block;}}"
        );
    }
}
